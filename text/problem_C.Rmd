...
\begin{equation}
\hat{\lambda}_0 = n \left(\sum_{i=1}^{n} \left(u_i z_i + (1 - u_i) \left(\frac{1}{\lambda_0^{(t)}} - \frac{z_i}{\exp{\left\{\lambda_0^{(t)} z_i\right\}} - 1}\right)\right)\right)^{-1}
\end{equation}

and

\begin{equation}
\hat{\lambda}_1 = n \left(\sum_{i=1}^{n} \left((1 - u_i) z_i + u_i \left(\frac{1}{\lambda_1^{(t)}} - \frac{z_i}{\exp{\left\{\lambda_1^{(t)} z_i\right\}} - 1}\right)\right)\right)^{-1}.
\end{equation}

Starting out with some intial values $\lambda_0^{(0)}$ and $\lambda_1^{(0)}$, we are know able to implement an EM algorithm by iteratively calculating $\lambda_0^{(t+1)} = \hat{\lambda}_0$ and $\lambda_1^{(t+1)} = \hat{\lambda}_1$ using $\lambda_0^{(t)}$ and $\lambda_1^{(t)}$, and then using these new values in the next iteration step. The initial values should be chosen among legal values -- that is, positive values -- but except from this restriction, the actual choice of initial values does not really matter. The function `EM()` below returns a `data.frame` object with all the $\lambda_0^{(t)}$ and $\lambda_1^{(t)}$ for $t = 0,...,t_{max}$, with $t_{max}$ the number of iterations.
...
Running the code above with $\lambda_0^{(0)} = \lambda_1^{(0)} = 5$, 20 iterations, and `z` and `u` from the provided files `z.txt` and `u.txt`, the estimates of $\hat{\lambda}_0$ and $\hat{\lambda}_1$ becomes `r sprintf("%.3f", em_estimate$lambda0[iterations+1])` and `r sprintf("%.3f", em_estimate$lambda1[iterations+1])` respectively. Figure \@ref(fig:emEstimate) shows that convergence happens almost instantly, and that about 10 iterations are in fact sufficient.

(ref:emEstimate) Plots showing the convergence of $\hat{\lambda}_0$ and $\hat{\lambda}_1$ with number of iterations on the x-axis and the values of the variables on the y-axis.
...
`for b in 1 to B:`$\\$
  ...
  $\hspace*{10mm}$`Run the EM function for this sample`$\\$
  $\hspace*{10mm}$`Append the converged (final) values to lambda0 and lambda1`$\\$
`end for`

`Denote by` $\hat{\lambda}_0^{(b)}$ `element number b in lambda0`$\\$
`Denote by` $\bar{\lambda}_0^{(B)}$ `the mean of the elements in lambda0`$\\$
`Do the exact same for lambda1`$\\$

$\hat{\sigma}_{\hat{\lambda}_0}$ `<-` $\sqrt{\frac{1}{B-1} \sum_{b=1}^{B} (\hat{\lambda}_0^{(b)} - \bar{\lambda}_0^{(B)})}\\$
$\hat{\sigma}_{\hat{\lambda}_1}$ `<-` $\sqrt{\frac{1}{B-1} \sum_{b=1}^{B} (\hat{\lambda}_1^{(b)} - \bar{\lambda}_1^{(B)})}\\$
$\text{Bias}(\lambda_0^{*})$
