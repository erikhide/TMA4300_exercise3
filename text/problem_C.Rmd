...
Using $u_i$ as an indicator, and realising that for case _ii)_ the result is in some sense symmetric, the expected value of the log-likelihood function given $z$, $u$, $\lambda_0^{(t)}$ and $\lambda_1^{(t)}$ becomes

\begin{align*}
E\left[n (\ln(\lambda_0) + \ln(\lambda_1)) - \lambda_0 \sum_{i=1}^{n} x_i - \lambda_1 \sum_{i=1}^{n} y_i | z, u, \lambda_0^{(t)}, \lambda_1^{(t)}\right] &= n (\ln(\lambda_0) + \ln(\lambda_1))\\
...
We want to find the values of $\lambda_0$ and $\lambda_1$ for which the result from 1 is maximised. By differentiating the expression with respect to $\lambda_0$ and setting it equal to 0, and then doing the same for $\lambda_1$, we get

\begin{equation}
\widehat{\lambda}_0 = n \left(\sum_{i=1}^{n} \left(u_i z_i + (1 - u_i) \left(\frac{1}{\lambda_0^{(t)}} - \frac{z_i}{\exp{\left\{\lambda_0^{(t)} z_i\right\}} - 1}\right)\right)\right)^{-1}
\end{equation}

and

\begin{equation}
\widehat{\lambda}_1 = n \left(\sum_{i=1}^{n} \left((1 - u_i) z_i + u_i \left(\frac{1}{\lambda_1^{(t)}} - \frac{z_i}{\exp{\left\{\lambda_1^{(t)} z_i\right\}} - 1}\right)\right)\right)^{-1}.
\end{equation}

Starting out with some intial values $\lambda_0^{(0)}$ and $\lambda_1^{(0)}$, we are know able to implement an EM algorithm by iteratively calculating $\lambda_0^{(t+1)} = \widehat{\lambda}_0$ and $\lambda_1^{(t+1)} = \widehat{\lambda}_1$ using $\lambda_0^{(t)}$ and $\lambda_1^{(t)}$, and then using these new values in the next iteration step. The initial values should be chosen among legal values -- that is, positive values -- but except from this restriction, the actual choice of initial values does not really matter for this specific problem, as the convergence properties are exceptionally good. However, EM algorithms in general tend to be quite slow, and hence good (i.e. more correct) starting values should be preferred in order to reduce the time until convergence. One possible way to get good initial values in this problem might be to calculate the mean of the $z$ values for which $u$ equals 1 and use the inverse of that as $\lambda_0^{(0)}$ -- the inverse of the mean of the remaining $z$ values should then become our $\lambda_1^{(0)}$. The function `EM()` below returns a `data.frame` object with all the $\lambda_0^{(t)}$ and $\lambda_1^{(t)}$ for $t = 0,...,t_{max}$, with $t_{max}$ the number of iterations.
...
(ref:emEstimate) Plots showing the convergence of $\widehat{\lambda}_0$ and $\widehat{\lambda}_1$ with number of iterations on the x-axis and the values of the variables on the y-axis.
...
$\widehat{\text{se}}\left(\widehat{\lambda}_0\right)$ `<-` $\sqrt{\frac{1}{B-1} \sum_{b=1}^{B} \left(\widehat{\lambda}_0^{*(b)} - \widehat{\lambda}_0^{*(\cdot)}\right)}\\$
$\vspace{4mm}$
$\widehat{\text{se}}\left(\widehat{\lambda}_1\right)$ `<-` $\sqrt{\frac{1}{B-1} \sum_{b=1}^{B} \left(\widehat{\lambda}_1^{*(b)} - \widehat{\lambda}_1^{*(\cdot)}\right)}\\$
$\vspace{4mm}$
...
$\vspace{4mm}$
$\widehat{\text{corr}}\left(\widehat{\lambda}_0, \widehat{\lambda}_1\right)$ `<-` $\frac{\frac{1}{B-1} \sum_{b=1}^{B} \left(\widehat{\lambda}_0^{*(b)} - \widehat{\lambda}_0^{*(\cdot)}\right) \left(\widehat{\lambda}_1^{*(b)} - \widehat{\lambda}_1^{*(\cdot)}\right)}{\widehat{\text{se}}\left(\widehat{\lambda}_0\right) \cdot  \widehat{\text{se}}\left(\widehat{\lambda}_1\right)}\\$
...
The function `bootstrap()` is an implementation of this pseudocode. Running the function with intial value of 5 for $\lambda_0^{(0)}$ and $\lambda_1^{(0)}$, 20 iterations of the EM algorithm per bootstrap sample, and 1000 bootstrap samples in total, gives $\widehat{\text{se}}\left(\widehat{\lambda}_0\right) = `r sprintf("%.3f", bootstrapRun$se_lambda0)`$, $\widehat{\text{se}}\left(\widehat{\lambda}_1\right) = `r sprintf("%.3f", bootstrapRun$se_lambda1)`$, $\widehat{\text{bias}}\left(\widehat{\lambda}_0\right) = `r sprintf("%.3f", bootstrapRun$bias_lambda0)`$, $\widehat{\text{bias}}\left(\widehat{\lambda}_1\right) = `r sprintf("%.3f", bootstrapRun$bias_lambda1)`$, and $\widehat{\text{corr}}\left(\widehat{\lambda}_0, \widehat{\lambda}_1\right) = `r sprintf("%.3f", bootstrapRun$corr)`$.
